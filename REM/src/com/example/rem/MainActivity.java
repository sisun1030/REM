...
public class MainActivity extends Activity {
     ...
     * classes.
     */
	public static final String LOG_TAG = "Teddy";
...
*/
    ...
    public void append(byte[] buffer, int base, int length) {
        for (int i = 0; i < length; i++) {
            ...
            try {
                if (MainActivity.LOG_CHARACTERS_FLAG) {
                    char printableB = (char) b;
                    if (b < 32 || b > 126) {
                        printableB = ' ';
                    }
                    Log.w(MainActivity.LOG_TAG, "'" + Character.toString(printableB)
                            + "' (" + Integer.toString(b) + ")");
                }
                process(b);
                mProcessedCharCount++;
            } catch (Exception e) {
                Log.e(MainActivity.LOG_TAG, "Exception while processing character "
    ...
    private void selectGraphicRendition() {
        for (int i = 0; i <= mArgIndex; i++) {
            ...
            if (code == 0) { // reset
                ...
                if (MainActivity.LOG_UNKNOWN_ESCAPE_SEQUENCES) {
                    Log.w(MainActivity.LOG_TAG, String.format("SGR unknown code %d", code));
                }
            }
        }
    }
...
class EmulatorView extends View implements GestureDetector.OnGestureListener {
    ...
    private MainActivity mBlueTerm;
...
*/
    ...
    public void initialize(MainActivity MainActivity) {
    	mBlueTerm = MainActivity;
        mTextSize = 15;
        mForeground = MainActivity.WHITE;
        mBackground = MainActivity.BLACK;
        updateText();
